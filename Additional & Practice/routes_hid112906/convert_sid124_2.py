...
def create_list():
    with open("temp1_sid124.csv") as csv_file:

        f = open("temp2_sid124.txt", "a")

        dict_reader = DictReader(csv_file)
        
        for row in dict_reader:      
            admittime = row['hadm_id']  +","+  row['subject_id'] +","+ row['admittime'] +","+ row['admission_location'] +" admission"
            f.write(admittime)
            f.write("\n")
...
def write_to_csv():
    ...
    with open("temp2_sid124.txt", "r") as infile:
        stripped = (line.strip() for line in infile)
        lines = (line.split(",") for line in stripped if line)
        with open("sid124_event_log_2.csv", "w") as outfile:
