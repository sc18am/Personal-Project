...
def create_list():
    with open("temp_hid112906.csv") as csv_file:

        f = open("temp1_hid112906.txt", "a")

        dict_reader = DictReader(csv_file)
        
        for row in dict_reader:      
            admittime = row['hadm_id']  +","+  row['admittime'] +","+ row['admission_location'] #+","+'admittime'
            f.write(admittime)
            f.write("\n")

            dischtime = row['hadm_id']  +","+  row['dischtime'] +","+ 'dischtime'
            f.write(dischtime)
            f.write("\n")

            if len(row['deathtime']) != 0:  
                deathtime = row['hadm_id']  +","+  row['deathtime'] +","+ 'deathtime'
                f.write(deathtime)
                f.write("\n")
...
def write_to_csv():
    ...
    with open("temp1_hid112906.txt", "r") as infile:
        stripped = (line.strip() for line in infile)
        lines = (line.split(",") for line in stripped if line)
        with open("hid112906_event_log.csv", "w") as outfile:
            writer = csv.writer(outfile)
            writer.writerow(("hadm_id", "timestamp", "place"))
