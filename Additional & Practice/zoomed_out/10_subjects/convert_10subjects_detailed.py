...
def create_list():
    with open("10subjects_detailed.csv") as csv_file:

        f = open("10subjects_detailed.txt", "a")
        ...
        for row in dict_reader:      
            ...
            icustay =  row['hadm_id']  +","+ row['subject_id'] +","+ row['intime'] + "," + row['first_careunit'] + " icu stay"
            f.write(icustay)
            f.write("\n")
...
def write_to_csv():
    ...
    with open("10subjects_detailed.txt", "r") as infile:
        stripped = (line.strip() for line in infile)
        lines = (line.split(",") for line in stripped if line)
        with open("10subjects_detailed_event_log.csv", "w") as outfile:
