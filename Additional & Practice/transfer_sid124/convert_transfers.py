...
def create_list():
    with open("transfer_sid124.csv") as csv_file:
        ...
        """
        for row in dict_reader:       
            intime = row['hadm_id']  + "," + row['subject_id']  + "," +  row['intime'] + "," + row['prev_careunit']  + "," + row['curr_careunit']  + ","  + 'intime'
            f.write(intime)
            f.write("\n")

            outtime = row['hadm_id']  + "," + row['subject_id']  + "," +  row['outtime'] + "," + row['prev_careunit']  + ","  + row['curr_careunit']  + ","  + 'outtime'
            f.write(outtime)
            f.write("\n")

def write_to_csv():
    
    with open("temp_sid124.txt", "r") as infile:
        stripped = (line.strip() for line in infile)
        lines = (line.split(",") for line in stripped if line)
        with open("sid124_event_log.csv", "w") as outfile:
            writer = csv.writer(outfile)
            writer.writerow(("hadm_id", "subject_id", "timestamp", "previous_careunit", "current_careunit", "time"))
