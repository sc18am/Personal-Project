...
def create_list():
    with open("3tables.csv") as csv_file:
        ...
        for row in dict_reader:      
            admittime = row['hadm_id']  +","+  row['subject_id'] +","+ row['admittime'] +","+  row['admission_,location'] 
            f.write(admittime)
            f.write("\n")
            
            icustay =  row['hadm_id']  +","+ row['subject_id'] +","+ row['intime']  +","+ row['icu_id'] + ", icu stay"
            f.write(icustay)
            f.write("\n")
            ...
            service = row['hadm_id']  +","+  row['subject_id'] +","+ row['transfer_time'] +","+  row['current_service']
