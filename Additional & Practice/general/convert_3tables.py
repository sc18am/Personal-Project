...
def create_list():
    with open("3tables.csv") as csv_file:

        f = open("3tables.txt", "a")
        ...
        for row in dict_reader:      
            ...
            if len(row['icustay_id']) != 0:
                icustay =  row['hadm_id']  +","+ row['subject_id'] +","+ row['intime']  +","+ row['icustay_id'] + " icu stay"
                f.write(icustay)
                f.write("\n")
            ...
            service = row['hadm_id']  +","+  row['subject_id'] +","+ row['transfertime'] +","+  row['curr_service']
            f.write(service)
            f.write("\n")
...
def write_to_csv():
    ...
    with open("3tables.txt", "r") as infile:
        stripped = (line.strip() for line in infile)
        lines = (line.split(",") for line in stripped if line)
        with open("3tables_event_log.csv", "w") as outfile:
