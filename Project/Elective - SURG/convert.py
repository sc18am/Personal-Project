...
def create_list():
    with open("elective-surg.csv") as csv_file:
        ...
        for row in dict_reader:    
            ...
            if len(row['outtime']) != 0:  
                ...
                if row['transfertime'] == row['intime'] and len(row['curr_careunit']) != 0 and row['curr_careunit'] == "SICU":
                ...
                elif len(row['icustay_id']) != 0 and row['curr_careunit'] == "SICU":
                    icustay =  row['hadm_id']  +","+ row['subject_id'] +","+ row['intime']  + "," + row['curr_careunit']  
                    f.write(icustay)
                    f.write("\n")
